...
function Get-OutputFile([string]$InputFile) {
    ...
    $n = 2
    do {
        $candidate = Join-Path $folder ($base + $suffix + "_" + $n + ".mp4")
        $n++
...
function Build-FFmpegArguments([string]$InputFile, [string]$OutputFile) {
    ...
    $args = @("-y","-hide_banner","-i",$InputFile,"-map","0:v:0","-map","0:a?","-sn","-dn")
...
$script:profiles = [ordered]@{
    "TV vieja (predeterminado)" = @{
        ...
        StripMetadata=$true; FastStart=$true; Suffix="_compatible"
        Description="Perfil probado en la TV antigua: H.264 Baseline 3.1, 1280x720, 30 FPS, YUV420p y AAC 44.1 kHz."
    }
    "TV HD moderna" = @{
        ...
        StripMetadata=$true; FastStart=$true; Suffix="_tv"
        Description="H.264 High 4.1 para televisores modernos con reproducción MP4 estándar."
    }
    "TV Full HD 60 FPS" = @{
        Width=1920; Height=1080; FPS="60"; VideoCodec="H.264 (AVC)"; Profile="High"; Level="4.2"; CRF=20
        ScaleMode="Ajustar + barras"; AudioCodec="AAC"; AudioBitrate="192k"; SampleRate="48000"; Channels="2"
        StripMetadata=$true; FastStart=$true; Suffix="_1080p60"
        Description="H.264 High 4.2 a 1080p60 para TVs que soportan 60 FPS."
    }
    "Android / tablets" = @{
        ...
        StripMetadata=$true; FastStart=$true; Suffix="_android"
        Description="MP4 H.264 + AAC para reproducción amplia en Android y tablets."
    }
    "iPhone / iPad" = @{
        Width=1920; Height=1080; FPS="30"; VideoCodec="H.264 (AVC)"; Profile="High"; Level="4.1"; CRF=20
        ScaleMode="Ajustar + barras"; AudioCodec="AAC"; AudioBitrate="160k"; SampleRate="48000"; Channels="2"
        StripMetadata=$true; FastStart=$true; Suffix="_ios"
        Description="H.264 + AAC en MP4 para compatibilidad amplia en dispositivos Apple."
    }
    "Web / redes" = @{
        Width=1280; Height=720; FPS="30"; VideoCodec="H.264 (AVC)"; Profile="High"; Level="3.1"; CRF=22
        ScaleMode="Ajustar + barras"; AudioCodec="AAC"; AudioBitrate="128k"; SampleRate="48000"; Channels="2"
        StripMetadata=$true; FastStart=$true; Suffix="_web"
        Description="Archivo H.264 ligero para web, mensajería y redes sociales."
    }
    "TV muy antigua 480p" = @{
        ...
        StripMetadata=$true; FastStart=$true; Suffix="_480p"
        Description="Perfil de compatibilidad máxima con resolución 480p para reproductores antiguos."
    }
    "HEVC / H.265" = @{
        Width=1920; Height=1080; FPS="30"; VideoCodec="H.265 (HEVC)"; Profile="Main"; Level="4.1"; CRF=24
        ScaleMode="Ajustar + barras"; AudioCodec="AAC"; AudioBitrate="160k"; SampleRate="48000"; Channels="2"
        StripMetadata=$true; FastStart=$true; Suffix="_hevc"
        Description="H.265/HEVC para dispositivos que soportan HEVC; genera archivos más pequeños."
    }
}
...
$codecCombo = New-Combo 285 55 125 @("H.264 (AVC)","H.265 (HEVC)","MPEG-4 Part 2")
...
$audioCombo = New-Combo 18 120 90 @("AAC","MP3","Copy","Sin audio")
...
$btnConvert.Add_Click({
    ...
    foreach ($input in $script:files) {
        ...
        try {
            ...
            elseif ($exitCode -eq 0 -and (Test-Path -LiteralPath $output)) {
                ...
                $errorCount++
                Log "ERROR: $base (código $exitCode)"
            ...
            $errorCount++
            Log "EXCEPCIÓN: $($_.Exception.Message)"
        }
        finally {
    ...
    if ($script:cancelRequested) {
        $lblProgress.Text = "Conversión cancelada."
    } else {
...
if (-not $script:ffmpeg) {
    Update-FFmpegStatus
    Log "FFmpeg no encontrado. La herramienta solicitará su descarga automática."
    [System.Windows.Forms.Application]::DoEvents()
    $null = Download-FFmpeg
}
else {
